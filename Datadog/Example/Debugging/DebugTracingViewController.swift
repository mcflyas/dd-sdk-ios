...
class DebugTracingViewController: UIViewController {
    ...
    @IBAction func didTapSendSingleSpan(_ sender: Any) {
        ...
        queue1.async {
            let span = GlobalDatadog.sharedTracer.startSpan(operationName: spanName)
    ...
    @IBAction func didTapSendComplexSpan(_ sender: Any) {
        ...
        queue1.async { [weak self] in
            ...
            let rootSpan = GlobalDatadog.sharedTracer.startSpan(operationName: spanName)
            wait(seconds: 0.5)

            self.queue2.sync {
                let child1 = GlobalDatadog.sharedTracer.startSpan(operationName: "child operation 1", childOf: rootSpan.context)
                wait(seconds: 0.5)
                child1.finish()

                wait(seconds: 0.1)

                let child2 = GlobalDatadog.sharedTracer.startSpan(operationName: "child operation 2", childOf: rootSpan.context)
                wait(seconds: 0.5)

                self.queue3.sync {
                    let grandChild = GlobalDatadog.sharedTracer.startSpan(operationName: "grandchild operation", childOf: child2.context)
                    wait(seconds: 1)
                    grandChild.finish()
                }
